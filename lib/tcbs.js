// lib/tcbs.js — Yahoo Finance source (server-side only, API routes)
// yahoo-finance2 v2.14+ dùng class pattern

import YahooFinance from "yahoo-finance2";

// Tạo instance đúng cách
let _yf = null;
function getYF() {
  if (!_yf) {
    _yf = new YahooFinance();
    // Suppress survey notice
    try { _yf.suppressNotices(["yahooSurvey"]); } catch {}
  }
  return _yf;
}
...
export async function getHistory(ticker, days = 800) {
  ...
  if (!result || result.length === 0) {
    throw new Error(`Không có lịch sử giá cho ${t}`);
  }
